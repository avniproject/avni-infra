...
| Logo image | `assets/logo.png` (TANUH + Ministry of Education composite, 512×170 RGBA, derived from the tanuh.ai reference AVIF with the originally-white wordmark/divider/emblem/text recolored to `#1F4D4D` so it reads against Metabase's white chrome; the TANUH icon stays in its original blue/green palette) is copied into `resources/frontend_client/app/assets/img/logo.png` during build. This single file backs the header, the login screen, **and** the favicon. |
